...
fn get_busybox_applets(busybox_path: &std::path::Path) -> Result<Vec<String>> {
    ...
    if output.status.success() {
        ...
        return Ok(stdout.lines().map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect());
    }
...
const COMMON_APPLETS: &[&str] = &[
    // Core utilities
    "ash", "sh", "cat", "chgrp", "chmod", "chown", "cp", "date", "dd", "df",
    "dmesg", "echo", "false", "hostname", "kill", "ln", "login", "ls",
    "mkdir", "mknod", "mount", "mv", "pidof", "ps", "pwd", "rm", "rmdir",
    "sed", "sleep", "stat", "stty", "su", "sync", "true", "umount", "uname",
    // Extended utilities
    "awk", "base64", "basename", "bunzip2", "bzcat", "bzip2", "cal", "chroot",
    "clear", "cmp", "comm", "cut", "diff", "dirname", "du", "env", "expr",
    "find", "fold", "free", "grep", "gzip", "head", "hexdump", "id", "install",
    "killall", "less", "logger", "md5sum", "mkfifo", "mktemp", "more", "nc",
    "nohup", "od", "patch", "pgrep", "ping", "ping6", "pkill", "printf",
    "readlink", "realpath", "renice", "rev", "seq", "sha1sum", "sha256sum",
    "sha512sum", "sort", "split", "strings", "tail", "tar", "tee", "test",
    "time", "timeout", "touch", "tr", "traceroute", "tty", "uniq", "unlink",
    "unxz", "unzip", "uptime", "vi", "watch", "wc", "wget", "which", "whoami",
    "xargs", "xz", "yes", "zcat",
    // Init and system
    "init", "halt", "poweroff", "reboot",
    // Networking
    "ifconfig", "ip", "route", "udhcpc", "ping",
    // sbin utilities
    "blkid", "fdisk", "fsck", "getty", "hwclock", "insmod", "klogd",
    "loadkmap", "losetup", "lsmod", "mdev", "mkfs.ext2", "mkfs.vfat",
    "mkswap", "modinfo", "modprobe", "pivot_root", "rmmod", "swapoff",
    "swapon", "switch_root", "sysctl", "syslogd",
];
